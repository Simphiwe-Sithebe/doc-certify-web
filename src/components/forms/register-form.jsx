  ...
  const { handleSubmit, handleChange, values, setFieldError, errors } =
    useFormik({
      ...
      onSubmit: (values) => {
        const newValues = { 
          ...values, 
          username: generateUsername(values.firstName.slice(0, 3), values.lastName.slice(0, 4))
        };
        AuthService.register(newValues)
          ...
          .catch(({ response }) => {
            ...
            if (error) {
              ...
              toast.error(email || phone || username || "Failed to create account. Please try again.", {
                duration: 3000,
              });
            }
          });
